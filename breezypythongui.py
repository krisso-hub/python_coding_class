...
class EasyFrame(Tkinter.Frame):
    ...
    def addListbox(self, row, column, rowspan = 1, columnspan = 1,
        ...
        frame = Tkinter.Frame(self)
        frame.grid(row = row, column = column, columnspan = columnspan, rowspan = 
rowspan,
                   sticky = N+S+E+W)
        self.columnconfigure(column, weight = 1)
        self.rowconfigure(row, weight = 1)
    ...
    def prompterBox(self, title = "", promptString = "", inputText = "", fieldWidth
= 20):
...
class EasyRadiobuttonGroup(Tkinter.Frame):
    ...
    def addRadiobutton(self, text, command = lambda : 0):
        ...
        button.grid(row = self._buttonRow, column = self._buttonColumn, sticky = 
N+W)
...
class PrompterBox(tkSimpleDialog.Dialog):
    ...
    @classmethod
    def prompt(cls, title = "", promptString = "", inputText = "", fieldWidth = 
20):
        """Creates and pops up an input dialog."""
        dlg = PrompterBox(Tkinter.Frame(), title, promptString, inputText, 
fieldWidth)
        return dlg.getText()
...
class EasyDialog(tkSimpleDialog.Dialog):
    ...
    def addRadiobuttonGroup(self, master, row, column,
                            rowspan = 1, columnspan = 1, orient = VERTICAL):
        """Creates and returns a radio button group."""
        return EasyRadiobuttonGroup(master, row, column, rowspan, columnspan, 
orient)
